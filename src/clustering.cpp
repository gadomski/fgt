...
Clustering::Clustering(const arma::mat& source, int K, double bandwidth, double epsilon)
    : m_source(source)
...
arma::mat Clustering::compute_C(const arma::vec& q) const
{
    ...
    for (arma::uword i = 0; i < m_source.n_rows; ++i)
    {
        ...
        arma::rowvec center_monomials = compute_monomials(dx / m_bandwidth, m_p_max);
        double f = q(i) * std::exp(-distance2 / h2);
        C.row(k) += f * center_monomials;
    }

    arma::rowvec constant_series = compute_constant_series(m_source.n_cols, m_p_max);
